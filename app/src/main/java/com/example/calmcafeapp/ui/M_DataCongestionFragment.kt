...
class M_DataCongestionFragment : BaseFragment<FragmentMDataCongestionBinding>(R.layout.fragment_m__data_congestion) {
    ...
    override fun initStartView() {
        ...
        // ViewModel 데이터 관찰
        viewModel.congestionData.observe(viewLifecycleOwner) { data ->
            if (data != null) {
                // 이미지 로드
                Glide.with(this)
                    .load(data.averageCongestionImageUrl) // 연령별 분석 이미지 URL
                    .into(binding.imageAge)

                Glide.with(this)
                    .load(data.busiestAndLeastBusyImageUrl) // 성별 분석 이미지 URL
                    .into(binding.imageGender)
            }
        }

        // 데이터 가져오기
        viewModel.fetchCongestionData()
    }
