    ...
    fun fetchCafeDetail(storeId: Int, userLatitude: Double, userLongitude: Double) {
        apiService.getCafeDetail(storeId, userLatitude, userLongitude).enqueue(object : Callback<CafeDetailResponse> {
            override fun onResponse(call: Call<CafeDetailResponse>, response: Response<CafeDetailResponse>) {
                if (response.isSuccessful && response.body() != null) {
                    val cafeDetailResult = response.body()?.result
                    _cafeDetail.value = cafeDetailResult
                    _recommendStoreList.value = cafeDetailResult?.recommendStoreResDtoList
                    _storeCongestionLevel.value = cafeDetailResult?.storeCongestionLevel
                    _userCongestionLevel.value = cafeDetailResult?.userCongestionLevel
